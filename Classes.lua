...
all:RegisterAuras( {
    ...
    -- 冰箱（寒冰屏障）可解除的debuff by圆圆20260205
    ice_block_removable = {
        generate = function ( t )
    ...
    tricks_of_the_trade_buff = {
        ...
        max_stack = 1,
        shared = "player",
        copy = { 57933, 59628 },    --时光服添加自己增伤buff 59628    
    },
...
all:RegisterAbilities( {
    ...
    global_thermal_sapper_charge = {
        ...
        usable = function ()
            ...
            if start1 + duration1 > query_time then 
                return false, "通用热力工程炸药CD中" 
            end
            -- 检查萨隆邪铁炸弹冷却
            local start2, duration2 = GetItemCooldown( 41119 )
            if start2 + duration2 > query_time then 
    ...
    saronite_bomb = {
        ...
        readyTime = function ()
            -- 只返回自身冷却（通用热力工程炸药的冷却不影响萨隆邪铁炸弹的就绪时间）
            local start1, duration1 = GetItemCooldown( 41119 )
            local cd1 = max( 0, start1 + duration1 - query_time )
            return cd1
        end,
